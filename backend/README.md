...
This FastAPI service is the foundation for the AI Autonomous Knowledge & Workflow Platform. It exposes chat, document ingestion, search, RAG, and a simple multi-step agent; future milestones will extend it with richer agent tooling, MCP-like integrations, and voice support.

## Project Structure

- `app/main.py` application factory and router wiring
- `app/api/routes` endpoint definitions (`health`, `chat`, `documents`, `search`, `agents`)
...
- `app/services` domain services (LLM stub, `DocumentService`, `SearchService`, `SimpleAgent`, etc.)
...
- `POST /api/v1/documents` �?" accepts multipart uploads (text, PDF, DOCX, images) and stores chunked content with embeddings via `DocumentService`. Response includes generated `id` and the chunk count.
...
- `POST /api/v1/agents/execute` for a simple multi-step agent. It uses the LLM plus a `document_search` tool to plan, retrieve relevant chunks, and return an answer together with a short reasoning trace.
